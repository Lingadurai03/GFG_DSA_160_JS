...
// Difficulty: EasyAccuracy: 55.49%Submissions: 76K+Points: 2
...
function solution(head) {
  ...
  while (fast && fast.next) {
    ...
    if (slow === fast) {
      return findFirst(head, fast);
    }
  }
  return -1;
}
function findFirst(head, fast) {
  while (head !== fast) {
    head = head.next;
    fast = fast.next;
  }
...
console.log(solution(createLinkedList([1, 2, 3, 4, 5], 2)));
